...
import { HiChartPie, HiShoppingBag, HiInbox, HiUser, HiArrowSmRight, HiTable,HiCheck,HiHome,BsInstagram,BsGithub,BsTwitter,BsDribbble,HiAcademicCap } from 'react-icons/hi';
...
export default function RRresult() {
  ...
  useEffect(() => {
    if (apiData) {
      axios.get(`http://localhost:8080/api/getJOCHresult/${apiData?.username}`)
        .then(response => {
  ...
  <button
    onClick={(e) => {
      ...
      navigate('/RParticipantList');
    }}
    className="flex-grow px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-500"
  >
    see result
  </button>
</div>
