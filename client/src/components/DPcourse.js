...
import { CreatePGcourse } from '../helper/helper'
...
export default function DPcourse() {
...
const steps = [
    ...
    palette: {
      ...
      background: {
        ...
        const handleSubmit = () => {
          ...
          const values = {
         Name: localStorage.getItem('Gcourse') ,
         LearningpathName : localStorage.getItem('LearningpathName')  ,
         Points: Points ,
         username: apiData?.username
      };
      
          let registerPromise = CreatePGcourse(values)
  ...
  useEffect(() => {
      ...
      axios.get(`http://localhost:8080/api/Gcourse/${localStorage.getItem('Gcourse')}`)
        .then(response => {
  ...
  function courseP() {
      const p = (Gcourse.CourseP);
      console.log(Gcourse.CourseP)
        if(QTC === 0){
        setPoints(prevPoints => prevPoints + p);
        setQTC(QTC+1);
        }
    ...
    const handleChange = (option) => {
      ...
      const evaluate = () => {
        let  p = (Gcourse.QuizP); 
          const solution = Gcourse.S;
      
          if (selectedAnswers === solution) {
...
<ThemeProvider theme={theme}>
    ...
    <div className="bg-gray-800 text-white">
      <div className="max-w-lg mx-auto py-20 px-6 sm:px-8 lg:max-w-7xl lg:px-8">
        <h1 className="text-4xl font-extrabold tracking-tight">{Gcourse.Name}</h1>
     
        <h4 className="text-gray-300">{Gcourse.Description}</h4>

        <div   className="mx-auto py-10" style={{ pointerEvents: 'auto' }}>
  <YouTube videoId={Gcourse.Course} className="w-full h-full" style={{ top: '0', left: '0' }} />
</div>
  ...
  <button class="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-700" onClick={handleNextStep}>Next</button>
      ...
      <div className="bg-gray-800 text-white py-10 p-6 rounded-lg mb-4">
       <form onSubmit={handleSubmit1}>
           ...
           <div  className="mb-6">
             <h2 className="text-lg font-bold mb-2">{Gcourse.question}</h2>
             <div className="space-y-2">
               <label className="flex items-center">
                 <input
                   ...
                   name="answer"
                   value={Gcourse.A}
                   onChange={() => handleChange( Gcourse.A)}
                   className="mr-2"
                 />
                 <span className="text-gray-100">{Gcourse.A}</span>
               </label>
               <label className="flex items-center">
                 <input
                   type="radio"
                   value={Gcourse.B}
                   name="answer"
                   onChange={() => handleChange( Gcourse.B)}
                   className="mr-2"
                 />
                 <span className="text-gray-100">{Gcourse.B}</span>
               </label>
               <label className="flex items-center">
                 <input
                   type="radio"
                   name="answer"
                   value={Gcourse.C}
                   onChange={() => handleChange( Gcourse.C)}
                   className="mr-2"
                 />
                 <span className="text-gray-100">{Gcourse.C}</span>
               </label>
               <label className="flex items-center">
                 <input
                   type="radio"
                   name="answer"
                   value={Gcourse.D}
                   onChange={() => handleChange( Gcourse.D)}
                   className="mr-2"
                 />
                 <span className="text-gray-100">{Gcourse.D}</span>
               </label>
             </div>
           </div></div>
