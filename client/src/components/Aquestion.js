...
import { updateuser,deleteQuestion,Createquestion } from '../helper/helper'
...
export default function Aquestion() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      if (Object.keys(errors).length === 0) {
        ...
        try {
          // Perform the question creation operation
          await Createquestion(values);

          // Display success toast notification
          toast.success(<b>Question added successfully...!</b>);
        } catch (error) {
          // Display error toast notification
          toast.error(<b>Could not add the question...!</b>);
        }
      } else {
 ...
 let [isOpen, setIsOpen] = useState(false)
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:8080/api/AQuestion')
      .then(response => {
  ...
  <table class="table-auto w-full">
      <thead>
        <tr class="bg-gray-700">
          <th class="px-4 py-2 text-left text-xs font-medium text-gray-100 uppercase tracking-wider">Question</th>
          <th class="px-4 py-2 text-left text-xs font-medium text-gray-100 uppercase tracking-wider">Language</th>
      ...
      <tbody class="bg-gray-600 divide-y divide-gray-500">
        {Question.map((Question) => (
          <tr key={Question._id} value={Question}>
            <td class="px-4 py-2 whitespace-nowrap">
              <div class="flex items-center">
                <div class="ml-4">
                  <div class="text-sm font-medium text-gray-100 whitespace-pre-wrap">{Question.Question}</div>
                </div>
              </div>
            </td>
            <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-400">{Question.Language}</td>
            <td class="px-4 py-2 whitespace-nowrap text-right text-sm font-medium">
              <a href="#" onClick={() => {
                            const value = { Question: Question.Question};
                            const values = Object.assign(value)
                            let registerPromise = deleteQuestion( values );
                              toast.promise(registerPromise, {
                                loading: 'Deleting...',
                                success : <b>Question Delete Successfully...!</b>,
                                error : <b>Could not Delete the Question.</b>
                              });
        ...
        <Dialog as="div" className="relative z-10" onClose={closeModal}>
          ...
          <option value="">Select a Language</option>
            ...
            <option value="Java">Java</option>
                ...
                <button type="submit" class="w-full px-5 py-3 text-base font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 sm:w-auto dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Create an Question</button>
