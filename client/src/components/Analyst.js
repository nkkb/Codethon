...
import { updatereport,createreport,deletreport } from '../helper/helper'
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8080/api/getreportA/${statistic}/${apiData?.username}`
        );
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      if (Object.keys(errors).length === 0) {
        ...
        try {
          // Perform the report creation operation
          await createreport(values);

          // Display success toast notification
          toast.success(<b>Report added Successfully...!</b>);
        } catch (error) {
          // Display error toast notification
          toast.error(<b>Could not add the report...!</b>);
        }
      } else {
  ...
  const formik1 = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      if (Object.keys(errors).length === 0) {
        // Submit the form and handle success/failure
        try {
          // Perform the report update operation
          await updatereport(values);

          // Display success toast notification
          toast.success(<b>Report updated successfully...!</b>);
        } catch (error) {
          // Display error toast notification
          toast.error(<b>Could not update the report...!</b>);
        }
      } else {
        // Display error toast notifications for each field
        Object.values(errors).forEach((errorMessage) => {
...
<Toaster position='top-center' reverseOrder={false}></Toaster>
  ...
  <div class="px-3 py-3 lg:px-5 lg:pl-3">
    ...
    >
      ...
      <Dropdown.Item>
      <Link to="/ANprofile"> Profile </Link>
      </Dropdown.Item>
      <Dropdown.Divider />
      <Dropdown.Item>
...
<main class="p-6 sm:p-10 space-y-6">
  ...
  <div  id="your-div-id-statistic4" class="p-4 flex-grow">
      ...
      <Tooltip />
      <Legend />
      <Bar dataKey="finishedJoboffer" fill="#82ca9d" />
      <Bar dataKey="unfinishedJoboffer" fill="#8884d8" />
    </BarChart></div>
          </div>
        </div>
...
<Transition appear show={isOpen} as={Fragment}>
  ...
  {reports.map((report) => (
    <div class="border-b border-gray-600 pb-4 mb-4 flex justify-between items-start" key={report.id}>
      ...
      <div class="flex items-center space-x-2">
        <button class="bg-yellow-500 text-white p-2 rounded" onClick={() => { 
          formik1.setFieldValue('title', report.title );
          formik1.setFieldValue('report', report.report );
          openModal3();
        }}>
        ...
        <button class="bg-red-500 text-white p-2 rounded" onClick={() => { 
      
            const value = { title: report.title};
            const values = Object.assign(value)
            let registerPromise = deletreport( values );
              toast.promise(registerPromise, {
                loading: 'reporting...',
                success : <b>report delete Successfully...!</b>,
                error : <b>Could not deleted the report.</b>
              });
        }}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
  ...
  <div class="flex justify-center mt-4">
    <button onClick={() => { 
      formik.setFieldValue('statistic', statistic );
      formik.setFieldValue('analyst', apiData?.username );
          openModal2();
        }}
    ...
    <PDFViewer width={600} height={600}>
      ...
      <div class="mb-4">
        ...
        <div class="mb-4">
          ...
          <textarea  rows={12} 
      cols={60}{...formik.getFieldProps('report')} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" id="Description" type="text" placeholder="report" />
        </div>
  ...
  <label class="block text-white font-bold text-2xl mb-2" for="Coursename">
    update
  </label>
</div>
          ...
          <div class="mb-4">
            ...
            <textarea  rows={12} 
        cols={60}{...formik1.getFieldProps('report')} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" id="Description" type="text" placeholder="report" />
          </div>
