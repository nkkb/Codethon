...
import { createPjobch } from '../helper/helper'
...
export default function DPjobchallenge() {
...
const steps = [
  ...
  const theme = createTheme({
    palette: {
      ...
      background: {
        ...
        const handleSubmit = (nbr) => {
          // Do something with formValues

          const values = {
            jobchallenge: Jobchallenge.Joboffername ,
            username: apiData?.username,
            NbrQ: Jobchallenge.QCM.length,
            NbrCAnswer: nbr
                };
      
          let registerPromise = createPjobch(values)
  ...
  useEffect(() => {
      ...
      axios.get(`http://localhost:8080/api/getJOchallenge/${localStorage.getItem('Joboffer')}`)
        .then(response => {
          setJobchallenge(response.data);
        })
        .catch(error => {
          console.log(error);
        });
  ...
  const handleChange = (index, option) => {
    setSelectedAnswers((prevAnswers) => {
      ...
      updatedAnswers[index] = {
        answer: option,
        solution: Jobchallenge.QCM[index].S,
      };
  ...
  const evaluate = () => {
    ...
    setTimeout(handleSubmit(count), 0);
  };
...
<ThemeProvider theme={theme}>
   ...
   <div className="bg-gray-800 text-white">
     <div className="max-w-lg mx-auto py-20 px-6 sm:px-8 lg:max-w-7xl lg:px-8">
       <h1 className="text-4xl font-extrabold tracking-tight">{Jobchallenge.Joboffername}</h1>
    
       <h4 className="text-gray-300">{Jobchallenge.Description}</h4> 
     </div>
   </div>
   </div>
     ...
     <div className="bg-gray-800 text-white py-10 p-6 rounded-lg mb-4">
        ...
        {Jobchallenge.QCM.map((QCM, index) => (
